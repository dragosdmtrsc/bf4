...
#include "frontends/p4/typeChecking/syntacticEquivalence.h"
...
public:
  ConstantTypeSubstitution(TypeVariableSubstitution *subst, ReferenceMap *,
                           TypeMap *typeMap, TypeInference *tc)
  ...
  convert(const IR::Vector<IR::Expression> *vec) {
...
const IR::Type *
TypeInference::cloneWithFreshTypeVariables(const IR::IMayBeGenericType *type) {
...
TypeInference::TypeInference(ReferenceMap *refMap, TypeMap *typeMap,
    ...
    : refMap(refMap), typeMap(typeMap),
      implMap(ImplementationMap::getCleared()), initialNode(nullptr),
      readOnly(readOnly) {
  CHECK_NULL(typeMap);
  CHECK_NULL(refMap);
...
const IR::Type *TypeInference::getType(const IR::Node *element) const {
  ...
  if ((result == nullptr) && (::errorCount() == 0))
    BUG("Could not find type of %1% (%2%)", element, (void *)element);
  return result;
}
...
const IR::Type *TypeInference::getTypeType(const IR::Node *element) const {
  ...
  BUG_CHECK(result->is<IR::Type_Type>(), "%1%: expected a TypeType",
            dbp(result));
...
TypeInference::canonicalizeFields(const IR::Type_StructLike *type) {
  ...
  for (auto field : type->fields) {
    ...
    if (ftype != field->type)
      changes = true;
    BUG_CHECK(!ftype->is<IR::Type_Type>(), "%1%: TypeType in field type",
              ftype);
    auto newField = new IR::StructField(field->srcInfo, field->name,
                                        field->annotations, ftype);
...
TypeInference::canonicalizeParameters(const IR::ParameterList *params) {
  if (params == nullptr)
    return params;
  ...
  for (auto p : *params->getEnumerator()) {
    ...
    if (paramType == nullptr)
      return nullptr;
    BUG_CHECK(!paramType->is<IR::Type_Type>(), "%1%: Unexpected parameter type",
              paramType);
    if (paramType != p->type) {
      p = new IR::Parameter(p->srcInfo, p->name, p->annotations, p->direction,
                            paramType, p->defaultValue);
      changes = true;
    }
    setType(p, paramType);
    vec->push_back(p);
...
bool TypeInference::checkParameters(const IR::ParameterList *paramList,
                                    bool forbidModules,
  ...
  for (auto p : paramList->parameters) {
    ...
    if (p->direction != IR::Direction::None && type->is<IR::Type_Extern>()) {
      typeError("%1%: a parameter with an extern type cannot have a direction",
                p);
      return false;
    }
 ...
 */
const IR::Type *
TypeInference::specialize(const IR::IMayBeGenericType *type,
                          const IR::Vector<IR::Type> *arguments) {
  TypeVariableSubstitution *bindings = new TypeVariableSubstitution();
  bool success = bindings->setBindings(type->getNode(),
                                       type->getTypeParameters(), arguments);
  if (!success)
    return nullptr;
...
const IR::Type *TypeInference::canonicalize(const IR::Type *type) {
  ...
  if (type->is<IR::Type_SpecializedCanonical>() ||
      type->is<IR::Type_InfInt>() || type->is<IR::Type_Action>() ||
      type->is<IR::Type_Error>() || type->is<IR::Type_Newtype>()) {
    return type;
    ...
    if (pl != tp->applyParams || tps != tp->typeParameters) {
      auto pl0 = pl->parameters.at(0);
      LOG1("pl0 = " << pl0 << " (" << ((void *)pl0) << ")");
      LOG1("old pl0 = " << tp->applyParams->parameters.at(0) << " ("
                        << ((void *)tp->applyParams->parameters.at(0)) << ")");
      return new IR::Type_Parser(tp->srcInfo, tp->name, tp->annotations, tps,
                                 pl);
    }
    return type;
    ...
    if (!checkParameters(pl, forbidModules, forbidPackages))
      return nullptr;
    if (pl != tp->applyParams || tps != tp->typeParameters)
      return new IR::Type_Control(tp->srcInfo, tp->name, tp->annotations, tps,
                                  pl);
    return type;
    ...
    if (pl != tp->constructorParams || tps != tp->typeParameters)
      return new IR::Type_Package(tp->srcInfo, tp->name, tp->annotations, tps,
                                  pl);
    return type;
    ...
    if (ctype != cont->type || pl != cont->constructorParams)
      return new IR::P4Control(cont->srcInfo, cont->name, ctype, pl,
                               cont->controlLocals, cont->body);
    return type;
    ...
    if (pl == nullptr)
      return nullptr;
    if (ctype != cont->type || pl != cont->constructorParams)
      return new IR::P4PackageModel(cont->srcInfo, cont->name, ctype, pl,
                                    cont->controlLocals, cont->body);
    return type;
    ...
    if (ctype != p->type || pl != p->constructorParams)
      return new IR::P4Parser(p->srcInfo, p->name, ctype, pl, p->parserLocals,
                              p->states);
    return type;
    ...
    if (fields != &hdr->fields)
      canon = new IR::Type_Header(hdr->srcInfo, hdr->name, hdr->annotations,
                                  *fields);
    else
      canon = hdr;
    return canon;
    ...
    if (fields != &str->fields)
      canon = new IR::Type_Struct(str->srcInfo, str->name, str->annotations,
                                  *fields);
    else
      canon = str;
    return canon;
    ...
    const IR::Type *canon;
    if (fields != &str->fields)
      canon = new IR::Type_HeaderUnion(str->srcInfo, str->name,
                                       str->annotations, *fields);
    else
      canon = str;
    return canon;
    ...
    if (tp->size() != st->arguments->size()) {
      typeError("%1%: Type %2% has %3% type parameter(s), but it is "
                "specialized with %4%",
    ...
    auto result = new IR::Type_SpecializedCanonical(type->srcInfo, baseCanon,
                                                    args, specialized);
...
const IR::Node *TypeInference::postorder(IR::P4Action *action) {
  ...
  for (auto p : action->parameters->parameters) {
    ...
    if (p->direction == IR::Direction::None)
      ...
      typeError("%1%: direction-less action parameters have to be at the end",
                p);
  }
  setType(getOriginal(), type);
  setType(action, type);
...
const IR::Node *TypeInference::postorder(IR::Declaration_Variable *decl) {
  ...
  if (type->is<IR::IContainer>() || type->is<IR::Type_Extern>()) {
    if (!findContext<IR::P4PackageModel>()) {
      typeError("%1%: cannot declare variables of type %2% (consider using an "
                "instantiation)",
                decl, type);
      return decl;
    }
...
bool TypeInference::canCastBetween(const IR::Type *dest,
                                   const IR::Type *src) const {
  if (src == dest)
    return true;
...
const IR::Expression *
TypeInference::assignment(const IR::Node *errorPosition,
                          const IR::Type *destType,
  ...
  if (destType->is<IR::Type_StructLike>() && initType->is<IR::Type_Tuple>() &&
    ...
    typeError("%1%: Cannot assign a %2% to a %3%", errorPosition, initType,
              destType);
    return sourceExpression;
  }
...
const IR::Node *TypeInference::postorder(IR::Declaration_Constant *decl) {
  ...
  if (!isCompileTimeConstant(decl->initializer))
    typeError("%1%: Cannot evaluate initializer to a compile-time constant",
              decl->initializer);
...
const IR::Vector<IR::Argument> *TypeInference::checkExternConstructor(
    const IR::Node *errorPosition, const IR::Type_Extern *ext,
    const IR::Vector<IR::Argument> *arguments) {
  auto tp = ext->getTypeParameters();
  if (!tp->empty()) {
    typeError("%1%: Type parameters must be supplied for constructor",
              errorPosition);
    return nullptr;
  }
  ...
  BUG_CHECK(methodType != nullptr,
            "Constructor does not have a method type, but %1%", mt);
  ...
  for (auto d : inst->initializer->components) {
    if (d->is<IR::Function>()) {
      ...
      if (func->type->typeParameters->size() != 0) {
        typeError("%1%: abstract method implementations cannot be generic",
                  func);
        return false;
      }
...
const IR::Node *TypeInference::preorder(IR::Declaration_Instance *decl) {
  ...
  auto type = getTypeType(decl->type);
  if (type && type->is<IR::P4Parser>()) {
    LOG1("decl type of " << decl << " = " << type);
    auto pt = type->to<IR::P4Parser>()->type;
    LOG1("type(decltype)=" << pt);
    LOG1("arg0(type(decltype))="
         << pt->applyParams->parameters.at(0) << " ("
         << ((void *)pt->applyParams->parameters.at(0)));
  }
  if (type == nullptr) {
    prune();
    return decl;
  ...
  if (simpleType->is<IR::Type_Extern>()) {
    ...
    if (decl->initializer != nullptr) {
      typeError("%1%: initializers only allowed for extern instances",
                decl->initializer);
      prune();
      return decl;
    }
    ...
    auto typeAndArgs = containerInstantiation(decl, decl->arguments,
  ...
  if (getType(decl)->is<IR::Type_Parser>()) {
    auto type = getType(decl);
    LOG1("setting type of " << decl->node_type_name() << " " << decl << " to "
                            << type << "(" << ((void *)type));
    auto tpp = type->to<IR::Type_Parser>();
    auto firstArg = tpp->applyParams->parameters.at(0);
    LOG1("first param " << firstArg << "(" << ((void *)firstArg));
  }
  prune();
  return decl;
}

/// @returns: A pair containing the type returned by the constructor and the new
/// arguments
...
TypeInference::containerInstantiation(
    const IR::Node
        *node, // can be Declaration_Instance or ConstructorCallExpression
  ...
  constructor =
      cloneWithFreshTypeVariables(constructor->to<IR::IMayBeGenericType>())
          ->to<IR::Type_Method>();
  ...
  // We build a type for the callExpression and unify it with the method
  // expression
  // Allocate a fresh variable for the return type; it will be hopefully bound
  // in the process.
  ...
  for (auto aarg : *constructorArguments) {
    ...
    if (argType == nullptr)
      return std::pair<const IR::Type *, const IR::Vector<IR::Argument> *>(
          nullptr, nullptr);
    auto argInfo =
        new IR::ArgumentInfo(arg->srcInfo, arg, true, argType, aarg->name);
    args->push_back(argInfo);
  }
  auto rettype = new IR::Type_Var(IR::ID(refMap->newName("R")));
  // There are never type arguments at this point; if they exist, they have been
  // folded
  ...
  BUG_CHECK(returnType != nullptr, "Cannot infer constructor result type %1%",
            node);
  return std::pair<const IR::Type *, const IR::Vector<IR::Argument> *>(
      returnType, newArgs);
}
...
const IR::Node *TypeInference::postorder(IR::Type_Newtype *type) {
  ...
  if (!argType->is<IR::Type_Bits>() && !argType->is<IR::Type_Boolean>() &&
    ...
    ::error("%1%: `type' can only be applied to base types", type);
  return type;
}
...
const IR::Node *TypeInference::postorder(IR::Type_Stack *type) {
  ...
  if (!etype->is<IR::Type_Header>() && !etype->is<IR::Type_HeaderUnion>())
    typeError("Header stack %1% used with non-header type %2%", type,
              etype->toString());
  return type;
}

void TypeInference::validateFields(
    const IR::Type *type, std::function<bool(const IR::Type *)> checker) const {
  if (type == nullptr)
    return;
  BUG_CHECK(type->is<IR::Type_StructLike>(), "%1%; expected a Struct-like",
            type);
  ...
  for (auto field : strct->fields) {
    ...
    if (ftype == nullptr)
      return;
    if (!checker(ftype))
      typeError("Field %1% of %2% cannot have type %3%", field,
  ...
  for (auto field : type->fields) {
    ...
    if (ftype->is<IR::Type_Varbits>()) {
      if (varbit == nullptr) {
        ...
        typeError("%1% and %2%: multiple varbit fields in a header", varbit,
                  field);
        return type;
      }
    }
  ...
  if (equTest) {
    ...
    if (TypeMap::equivalent(ltype, rtype) &&
      ...
      if ((ltype->is<IR::Type_StructLike>() && rtype->is<IR::Type_Tuple>() &&
           expression->right->is<IR::ListExpression>()) ||
          (ltype->is<IR::Type_Tuple>() &&
           expression->left->is<IR::ListExpression>() &&
    ...
    if (!defined) {
      typeError("%1%: not defined on %2% and %3%", expression,
...
const IR::Node *TypeInference::postorder(IR::Concat *expression) {
  ...
  const IR::Type *resultType =
      IR::Type_Bits::get(bl->size + br->size, bl->isSigned);
...
const IR::Node *TypeInference::postorder(IR::Key *key) {
  ...
  // installing also for the original because we cannot tell which one will
  // survive in the ir
...
const IR::Node *TypeInference::preorder(IR::EntriesList *el) {
  ...
  if (keyTuple == nullptr) {
    ...
    if (key->srcInfo.isValid() && el->srcInfo.isValid() &&
        key->srcInfo >= el->srcInfo) {
...
const IR::Node *TypeInference::postorder(IR::Entry *entry) {
  ...
  if (keyset == nullptr || !keyset->is<IR::ListExpression>()) {
    ...
    typeError("%1%: Size of entry keyset must match the table key set size",
              keyset);
    return entry;
  }
...
const IR::Node *
TypeInference::postorder(IR::StructInitializerExpression *expression) {
  if (done())
    return expression;
  bool constant = true;
...
const IR::Node *TypeInference::postorder(IR::ArrayIndex *expression) {
  ...
  if (!ltype->is<IR::Type_Stack>()) {
    typeError("Array indexing %1% applied to non-array type %2%", expression,
              ltype->toString());
    return expression;
  }
  ...
  if (rightOpConstant) {
    ...
    if (hst->sizeKnown()) {
      ...
      if (index >= size) {
        typeError("Array index %1% larger or equal to array size %2%", cst,
                  hst->size);
        return expression;
      }
    }
...
const IR::Node *
TypeInference::binaryBool(const IR::Operation_Binary *expression) {
  if (done())
    return expression;
  auto ltype = getType(expression->left);
  auto rtype = getType(expression->right);
...
const IR::Node *
TypeInference::binaryArith(const IR::Operation_Binary *expression) {
  if (done())
    return expression;
  auto ltype = getType(expression->left);
  auto rtype = getType(expression->right);
  ...
  if (bl != nullptr && br != nullptr) {
    if (bl->size != br->size) {
      typeError(
          "%1%: Cannot operate on values with different widths %2% and %3%",
          expression, bl->size, br->size);
      return expression;
    }
    if (bl->isSigned != br->isSigned) {
      typeError("%1%: Cannot operate on values with different signs",
                expression);
      return expression;
    }
...
const IR::Node *
TypeInference::unsBinaryArith(const IR::Operation_Binary *expression) {
  if (done())
    return expression;
  auto ltype = getType(expression->left);
  auto rtype = getType(expression->right);
  ...
  if (expression->right->is<IR::Constant>()) {
    ...
    if (lt != nullptr && shift >= lt->size)
      ::warning(ErrorType::WARN_OVERFLOW,
                "%1%: shifting value with %2% bits by %3%", expression,
                lt->size, shift);
  }
...
const IR::Node *TypeInference::bitwise(const IR::Operation_Binary *expression) {
  ...
  if (bl != nullptr && br != nullptr) {
    if (!TypeMap::equivalent(bl, br)) {
      typeError(
          "%1%: Cannot operate on values with different types %2% and %3%",
...
const IR::Node *TypeInference::postorder(IR::Cast *expression) {
  ...
  if (!canCastBetween(castType, sourceType)) {
    ...
    if (!canCastBetween(castType, sourceType))
      typeError("%1%: Illegal cast from %2% to %3%", expression,
                sourceType->toString(), castType->toString());
  }
...
const IR::Node *TypeInference::postorder(IR::PathExpression *expression) {
  ...
  if (type->is<IR::Type_Parser>()) {
    LOG1("setting type of " << expression << " to " << type << "("
                            << ((void *)type));
    auto tpp = type->to<IR::Type_Parser>();
    auto firstArg = tpp->applyParams->parameters.at(0);
    LOG1("first param " << firstArg << "(" << ((void *)firstArg));
...
const IR::Node *TypeInference::postorder(IR::Slice *expression) {
  ...
  const IR::Type *result =
      IR::Type_Bits::get(bst->srcInfo, m - l + 1, bst->isSigned);
...
const IR::Node *TypeInference::postorder(IR::Member *expression) {
  ...
  if (type->is<IR::Type_Extern>()) {
    ...
    // Each method invocation uses fresh type variables
    methodType =
        cloneWithFreshTypeVariables(methodType->to<IR::IMayBeGenericType>());
  ...
  if (type->is<IR::Type_StructLike>()) {
    if (type->is<IR::Type_Header>() || type->is<IR::Type_HeaderUnion>()) {
      if (inMethod && (member == IR::Type_Header::isValid)) {
        // Built-in method
        auto type = new IR::Type_Method(IR::Type_Boolean::get(),
    ...
    if (inMethod && (member == IR::Type_Header::minSizeInBits ||
      ...
      // Built-in method
      auto type =
          new IR::Type_Method(new IR::Type_InfInt(), new IR::ParameterList());
      auto ctype = canonicalize(type);
      if (ctype == nullptr)
        return expression;
      setType(getOriginal(), ctype);
  ...
  if (type->is<IR::IApply>() && member == IR::IApply::applyMethodName) {
    ...
    if (methodType == nullptr)
      return expression;
    // sometimes this is a synthesized type, so we have to crawl it to
    // understand it
    auto *learn = clone();
    ...
    if (member == IR::Type_Stack::next || member == IR::Type_Stack::last) {
      ...
      if (parser != nullptr)
        typeError("%1%: '%2%' and '%3%' for stacks cannot be used in a parser",
                  expression, IR::Type_Stack::push_front,
                  IR::Type_Stack::pop_front);
      if (!isLeftValue(expression->expr))
        typeError("%1%: must be applied to a left-value", expression);
      auto params = new IR::IndexedVector<IR::Parameter>();
      auto param = new IR::Parameter(IR::ID("count", nullptr),
                                     IR::Direction::In, new IR::Type_InfInt());
      ...
      return expression;
    } else if (inMethod && (member == IR::Type_StructLike::minSizeInBytes ||
...
TypeInference::actionCall(bool inActionList,
  ...
  for (auto arg : *actionCall->arguments) {
    ...
    if (named) {
      ...
      if (param == nullptr) {
        typeError("%1%: No parameter named %2%", baseType->parameters,
                  arg->name);
        return actionCall;
      }
    } else {
    ...
    BUG_CHECK(leftIt != left.end(), "%1%: Duplicate argument name?",
              param->name);
    left.erase(leftIt);
    ...
    if (param->direction == IR::Direction::None) {
      if (inActionList) {
        typeError("%1%: parameter %2% cannot be bound: it is set by the "
                  "control plane",
                  arg, param);
      } else if (inTable) {
  ...
  auto resultType =
      new IR::Type_Action(baseType->srcInfo, baseType->typeParameters, params);
  ...
  ConstantTypeSubstitution cts(tvs, refMap, typeMap, this);
  actionCall =
      cts.convert(actionCall)->to<IR::MethodCallExpression>(); // cast arguments
  if (::errorCount() > 0)
    return actionCall;
...
void TypeInference::checkEmitType(const IR::Expression *emit,
  ...
  typeError("%1%: argument must be a header, stack or union, or a struct or "
            "tuple of such types",
            emit);
}
...
const IR::Node *TypeInference::postorder(IR::MethodCallExpression *expression) {
  ...
  if (methodType->is<IR::Type_Action>()) {
    ...
    if (prop != nullptr &&
        prop->name == IR::TableProperties::actionsPropertyName)
      inActionsList = true;
    ...
    for (auto aarg : *expression->arguments) {
      ...
      if (argType == nullptr)
        return expression;
      auto argInfo =
          new IR::ArgumentInfo(arg->srcInfo, isLeftValue(arg),
    ...
    auto callType =
        new IR::Type_MethodCall(expression->srcInfo, typeArgs, rettype, args);
    ...
    if (returnType->is<IR::Type_Control>() ||
        returnType->is<IR::Type_Parser>() || returnType->is<IR::P4Parser>() ||
        returnType->is<IR::P4Control>() || returnType->is<IR::Type_Package>() ||
    ...
    ConstantTypeSubstitution cts(tvs, refMap, typeMap, this);
    auto result = cts.convert(expression)
...
const IR::Node *
TypeInference::postorder(IR::ConstructorCallExpression *expression) {
  if (done())
    return expression;
  ...
  if (simpleType->is<IR::Type_Extern>()) {
    auto args = checkExternConstructor(
        expression, simpleType->to<IR::Type_Extern>(), expression->arguments);
    if (args == nullptr)
      return expression;
    if (args != expression->arguments)
      expression = new IR::ConstructorCallExpression(
          expression->srcInfo, expression->constructedType, args);
    setType(getOriginal(), type);
    setType(expression, type);
  } else if (simpleType->is<IR::IContainer>()) {
    ...
    typeError("%1%: Cannot invoke a constructor on type %2%", expression,
              type->toString());
  }
...
static void convertStructToTuple(const IR::Type_StructLike *structType,
                                 IR::Type_Tuple *tuple) {
...
const IR::SelectCase *TypeInference::matchCase(
    const IR::SelectExpression *select, const IR::Type_Tuple *selectType,
  ...
  // The caseType may be a simple type, and then we have to unwrap the
  // selectType
...
const IR::Node *TypeInference::postorder(IR::This *expression) {
  ...
  if (findContext<IR::Function>() == nullptr || decl == nullptr)
    typeError("%1%: can only be used in the definition of an abstract method",
              expression);
  auto type = getType(decl);
  setType(expression, type);
  setType(getOriginal(), type);
...
const IR::Node *TypeInference::postorder(IR::SelectExpression *expression) {
  ...
  for (auto ct : tuple->components) {
    if (ct->is<IR::ITypeVar>()) {
      ...
      typeError("Expression with type %1% cannot be used in select %2%", ct,
                expression);
      return expression;
    }
  }
  ...
  if (changes)
    expression = new IR::SelectExpression(expression->srcInfo,
                                          expression->select, std::move(vec));
...
const IR::Node *TypeInference::postorder(IR::SwitchStatement *stat) {
  ...
  if (!type->is<IR::Type_ActionEnum>()) {
    typeError("%1%: Switch condition can only be produced by "
...
const IR::Node *TypeInference::postorder(IR::ReturnStatement *statement) {
  ...
  if (func == nullptr) {
    if (statement->expression != nullptr)
      typeError("%1%: return with expression can only be used in a function",
                statement);
    return statement;
  }
  ...
  BUG_CHECK(ftype->is<IR::Type_Method>(),
            "%1%: expected a method type for function", ftype);
  auto mt = ftype->to<IR::Type_Method>();
  auto returnType = mt->returnType;
  CHECK_NULL(returnType);
  if (returnType->is<IR::Type_Void>()) {
    if (statement->expression != nullptr)
      typeError("%1%: return expression in function with void return",
                statement);
    return statement;
  }
...
const IR::Node *TypeInference::postorder(IR::AssignmentStatement *assign) {
  ...
  if (!isLeftValue(assign->left)) {
    typeError("Expression %1% cannot be the target of an assignment",
              assign->left);
    LOG2(assign->left);
    return assign;
  }
  ...
  if (newInit != assign->right)
    assign =
        new IR::AssignmentStatement(assign->srcInfo, assign->left, newInit);
  return assign;
}
...
TypeInference::validateActionInitializer(const IR::Expression *actionCall,
                                         const IR::P4Table *table) {

  auto al = table->getActionList();
  if (al == nullptr) {
    typeError("%1% has no action list, so it cannot have %2%", table,
              actionCall);
    return nullptr;
  }
  ...
  for (auto param :
       *listInstance->substitution.getParametersInArgumentOrder()) {
...
const IR::Node *TypeInference::postorder(IR::Property *prop) {
  ...
  if (prop->name == IR::TableProperties::defaultActionPropertyName) {
    ...
    if (pv == nullptr) {
      ...
      BUG_CHECK(prop->value->is<IR::ExpressionValue>(), "%1% not an expression",
                prop);
